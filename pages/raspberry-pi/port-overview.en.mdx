...
| Port | Function |
| :--- | :--- |
| **8332** | The JSON-RPC port. With the right username/password combination, you can talk to this. By default, Bitcoin Knots only accepts connections from the Pi itself. Therefore, a tool running on the Pi will be allowed to talk to the RPC port, but a random attacker from the outside will not. |
...
| **9735** | The port which connects with other nodes. A bit like 8333 for Bitcoin Knots. |
